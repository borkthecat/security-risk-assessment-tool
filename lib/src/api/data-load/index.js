...
const DataLoad = (filePath) => {
  try {
    ...
    // Remove dangling vulnerability refs from attack paths (refs to deleted vulns).
    // The delete cascade can fail when riskIdRef is unavailable, leaving stale refs.
    const validVulIds = new Set(
      jsonData.Vulnerability
        .filter((v) => v && typeof v === 'object' && v.vulnerabilityId != null)
        .map((v) => v.vulnerabilityId)
    );
    jsonData.Risk.forEach((risk) => {
      if (!risk || !Array.isArray(risk.riskAttackPaths)) return;
      risk.riskAttackPaths.forEach((path) => {
        if (!Array.isArray(path.vulnerabilityRef)) return;
        path.vulnerabilityRef = path.vulnerabilityRef.filter(
          (ref) => ref != null && validVulIds.has(ref.vulnerabilityId)
        );
      });
    });
